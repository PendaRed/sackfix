...
object SfMissingFieldsException {
  def renderMsg(msg:String, missingTags:List[String]) = {
...
case class SfDuplicateTagIdException(val tagId:Int) extends Exception(s"Duplicate tagId($tagId) in message")
